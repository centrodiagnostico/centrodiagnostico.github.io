...
>
	...
	<xsl:template match="/*">
		<form class="d-flex" id="estudios_buscador" action="estudios.html?search={$searchParams:search}#estudios" target="_self" onsubmit="buscarEstudio">
			<input class="form-control me-2" type="search" placeholder="Buscar estudio..." aria-label="Buscar..." oninput="xo.site.searchParams.set('search', this.value)" value="{$searchParams:search}"/>
